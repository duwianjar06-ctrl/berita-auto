...
const fontPackageDir=path.dirname(require.resolve('@fontsource/noto-sans/package.json'));
const bundledRegularFont=require.resolve('@fontsource/noto-sans/files/noto-sans-latin-ext-400-normal.woff');
const bundledBoldFont=require.resolve('@fontsource/noto-sans/files/noto-sans-latin-ext-700-normal.woff');
const bundledFontDir=path.dirname(bundledRegularFont);
const regularPathExists=fs.existsSync(bundledRegularFont);
const boldPathExists=fs.existsSync(bundledBoldFont);
const bundledFontsReady=regularPathExists&&boldPathExists;
if(!bundledFontsReady) throw new Error('social_card_bundled_font_missing');
const runtimeFontconfig=path.join('/tmp','berita-auto-fonts.conf');
const fontconfigXml='<?xml version="1.0"?><!DOCTYPE fontconfig SYSTEM "fonts.dtd"><fontconfig><dir>'+bundledFontDir+'</dir><dir>'+path.join(fontPackageDir,'files')+'</dir><dir>/usr/share/fonts</dir><dir>/usr/local/share/fonts</dir><dir>/opt/fonts</dir><cachedir>/tmp/fontconfig</cachedir></fontconfig>';
try{
  fs.writeFileSync(runtimeFontconfig,fontconfigXml,'utf8');
  process.env.FONTCONFIG_FILE=runtimeFontconfig;
  process.env.FONTCONFIG_PATH=fontconfigDir;
}catch(error){
  throw new Error(`social_card_fontconfig_init_failed:${String(error?.message||error).slice(0,120)}`);
}
...
const safeText=value=>clip(value,1200);
...
export async function GET(request,{params}) {
  ...
  const content=socialCardText(article,slide)||{};
  const expectedTextLength=String(content.title||'').length+String(content.summary||'').length;
  const leadStarted=Date.now();
  const leadImage=slide===1?await fetchLeadImage(article):null;
  const renderStarted=Date.now();
  const svg=renderSvg(article,slide,slides,leadImage);
  const jpeg=await sharp(Buffer.from(svg)).jpeg({quality:86,mozjpeg:true}).toBuffer();
  console.log('[social-card-font] family=Noto Sans source=bundled regularPathExists=true boldPathExists=true regular='+bundledRegularFont+' bold='+bundledBoldFont);
  console.log('[social-card] articleId='+articleId+' slide='+slide+' image='+(leadImage?'article':'fallback')+' font=Noto-Sans leadMs='+(Date.now()-leadStarted)+' renderMs='+(Date.now()-renderStarted)+' totalMs='+(Date.now()-started));
  return new Response(jpeg,{status:200,headers:{
    'Content-Type':'image/jpeg',
    'Cache-Control':'public, max-age=300, s-maxage=300, stale-while-revalidate=600',
    'Content-Length':String(jpeg.length),
    'X-Social-Card-Render':'valid',
    'X-Social-Card-Text-Length':String(expectedTextLength),
    'X-Social-Card-Font-Source':'bundled'
  }});
}
