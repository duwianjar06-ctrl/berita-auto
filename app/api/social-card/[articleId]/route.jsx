...
export async function GET(request,{params}){
  const started=Date.now();const {articleId}=await params;const raw=(await readArticles()).find(item=>item.id===articleId);if(!raw)return new Response('Not found',{status:404});const article=normalizeSocialArticle(raw);const url=new URL(request.url);const slide=Math.max(1,Number(url.searchParams.get('slide')||1));const slides=buildSocialSlides(article);const content=socialCardText(article,slide)||{};const expectedTextLength=String(content.title||'').length+String(content.summary||'').length;const leadStarted=Date.now();const leadImage=slide===1?await fetchLeadImage(article):null;const renderStarted=Date.now();const svg=renderSvg(article,slide,slides,leadImage);const jpeg=await sharp(Buffer.from(svg)).jpeg({quality:86,mozjpeg:true}).toBuffer();if(jpeg.length<12*1024)throw new Error('social_card_render_too_small');console.log(`[social-card] articleId=${articleId} slide=${slide} image=${leadImage?'article':'fallback'} leadMs=${Date.now()-leadStarted} renderMs=${Date.now()-renderStarted} totalMs=${Date.now()-started}`);return new Response(jpeg,{status:200,headers:{'Content-Type':'image/jpeg','Cache-Control':'public, max-age=300, s-maxage=300, stale-while-revalidate=600','Content-Length':String(jpeg.length),'X-Social-Card-Render':'valid','X-Social-Card-Text-Length':String(expectedTextLength)}});
}
