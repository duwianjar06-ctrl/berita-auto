...
import {prepareInstagramReviewQueue} from '../../../../lib/instagram-review.js';
...
export async function GET(request){if(!authorized(request))return NextResponse.json({error:'Unauthorized'},{status:401});try{return NextResponse.json(await prepareInstagramReviewQueue({trigger:'social-prepare',now:Date.now()}),{status:200,headers:{'Cache-Control':'no-store'}})}catch(error){return NextResponse.json({status:'failed',error:String(error?.message||error).slice(0,240)},{status:503,headers:{'Cache-Control':'no-store'}})}}
