...
async function notifyPublication(result){const status=result?.status;if(!['published','failed','persist_error'].includes(status)||!result?.articleId)return;try{const articles=await readArticles();const article=articles.find(row=>row?.id===result.articleId)||null;const reportStatus=status==='published'?'success':status==='persist_error'?'partial':'failed';await sendTelegramPublicationReport({status:reportStatus,article,result,error:result?.reason?new Error(result.reason):null});}catch(error){console.warn(`[telegram] publication_notification_noncritical_failed articleId=${result?.articleId||'unknown'} reason=${String(error?.message||error).slice(0,180)}`);}}
async function execute(){const startedAt=new Date().toISOString();try{const result=await runSocialCycle({trigger:'vercel-cron',now:Date.now()});await recordSocialRun({triggerSource:'vercel-cron',startedAt,finishedAt:new Date().toISOString(),result}).catch(()=>{});await notifyPublication(result);return NextResponse.json(result,{status:200,headers:{'Cache-Control':'no-store'}});}catch(error){await recordSocialRun({triggerSource:'vercel-cron',startedAt,finishedAt:new Date().toISOString(),result:{status:'failed',reason:'exception'},error}).catch(()=>{});return NextResponse.json({status:'failed',error:String(error?.message||error).replace(/Bearer\s+\S+/gi,'Bearer [redacted]').slice(0,240)},{status:503,headers:{'Cache-Control':'no-store'}})}}
