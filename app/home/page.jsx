...
export default async function EditorialHome(){
 ...
 const imageFrequency=new Map();for(const article of visible){const image=imageKey(article);if(image)imageFrequency.set(image,(imageFrequency.get(image)||0)+1);}
 ...
 return <><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(schema)}}/><Header latestTitle={featured?.title||'Sistem sedang mencari berita terbaru…'}/><main className="page container">
  ...
  {featured?<section className="hero soft-scale" aria-label="Berita utama"><div><NewsCard article={featured}/></div><div className="side">{secondary.map((a,i)=><div key={a.id} style={{animationDelay:`${(i+1)*70}ms`}} className="fade-up"><NewsCard article={a}/></div>)}<AdSlot variant="rectangle" placement="homepage_sidebar"/></div></section>:<section className="empty"><h2>Belum ada berita terbaru</h2><p>Sistem sedang memeriksa sumber berita.</p></section>}
