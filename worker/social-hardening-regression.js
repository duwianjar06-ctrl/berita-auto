...
assert.match(route,/assets[\\/]fonts/);assert.match(route,/NotoSans-Regular\.ttf/);assert.match(route,/NotoSans-Bold\.ttf/);assert.match(route,/social_card_bundled_font_missing/);assert.match(route,/process\.env\.FONTCONFIG_FILE=runtimeFontconfig/);assert.match(route,/process\.env\.FONTCONFIG_PATH=fontconfigDir/);assert.match(route,/const \{default:sharp\}=await import\('sharp'\)/);assert.doesNotMatch(route,/@fontsource\/noto-sans/);assert.match(route,/source=bundled-ttf/);
