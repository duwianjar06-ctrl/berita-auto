...
export async function runPublicationCycle({trigger='manual',now=Date.now()}={}){
  ...
  try{
    ...
    let publishedNow=[...published];let publicationCount=0;let lastError=null;const publicationStart=Date.now();
    while(publicationCount<plan.maxPublish&&remaining.length){
      try{
        t=Date.now();await setPipelineStage(runId,'SELECTING',t);const chosen=selectPublication(remaining,publishedNow.slice(0,10),Date.now());await finishPipelineStage(runId,'SELECTING',t);if(!chosen)break;
        ...
        t=Date.now();await setPipelineStage(runId,'PARAPHRASING',t);const ai=await generateArticle(chosen);const aiDurationMs=Date.now()-t;console.log(`[perf] ai=${aiDurationMs}ms provider=${ai.generationProvider}`);await finishPipelineStage(runId,'PARAPHRASING',t);
        ...
        t=Date.now();await setPipelineStage(runId,'PUBLISHING',t);const sitePublishedAt=new Date().toISOString();const article={id:chosen.fingerprint,fingerprint:chosen.fingerprint,titleFingerprint:chosen.titleFingerprint,title:ai.title,summary:chosen.summary||ai.excerpt||chosen.title,excerpt:ai.excerpt||chosen.summary||chosen.title,content:ai.content,url:chosen.url,sourceUrl:chosen.sourceUrl,sourceName:chosen.sourceName,publisher:chosen.publisher||chosen.sourceName,sourceId:chosen.sourceId||null,category:chosen.category||'Nasional',imageUrl:image.imageUrl||chosen.imageUrl||null,imageSource:image.source||null,publishedAt:chosen.publishedAt||null,sourcePublishedAt:chosen.publishedAt||null,sitePublishedAt,createdAt:sitePublishedAt,generationProvider:ai.generationProvider||'fallback',generationModel:ai.generationModel||null,generationAt:ai.generationAt||sitePublishedAt,generationDurationMs:aiDurationMs};
        remaining=remaining.filter(item=>item.fingerprint!==chosen.fingerprint&&item.titleFingerprint!==article.titleFingerprint);publishedNow=[article,...publishedNow].filter((item,i,array)=>i===array.findIndex(x=>x.fingerprint===item.fingerprint)).slice(0,500);publicationCount++;await writeArticles(publishedNow);await writePending(remaining.slice(0,queueConfig.QUEUE_MAX));await finishPipelineStage(runId,'PUBLISHING',t);console.log(`[publish #${publicationCount}] publisher=${article.publisher} source=${article.sourceUrl} sourcePublishedAt=${article.sourcePublishedAt} sitePublishedAt=${article.sitePublishedAt}`);
      }catch(error){lastError=error;console.error(`[publish] attempt=${publicationCount+1} error=${String(error?.message||error).slice(0,240)}`);break;}
    }
