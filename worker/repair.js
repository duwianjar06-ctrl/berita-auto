...
if(!process.env.OPENAI_API_KEY){console.log(`[repair] candidate=${bad[0].id}; OPENAI_API_KEY missing; refusing non-paraphrased source replacement`);process.exit(2)}
const item=bad[0];
...
const repaired={...item,title:generated.title,excerpt:generated.excerpt,content:generated.content,updatedAt:new Date().toISOString()};
await writeArticles(items.map(x=>x.id===item.id?repaired:x));
console.log(`[repair] repaired=${item.id}`);
