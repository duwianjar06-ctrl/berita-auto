...
const commands=[];
globalThis.fetch=async(_url,options)=>{const args=JSON.parse(options.body);commands.push(args);const op=String(args[0]||'').toUpperCase();if(op==='MGET')return new Response(JSON.stringify({result:Array(Math.max(0,args.length-1)).fill(null)}),{status:200,headers:{'content-type':'application/json'}});if(op==='MSET'||op==='ZADD')return new Response(JSON.stringify({result:'OK'}),{status:200,headers:{'content-type':'application/json'}});throw new Error(`unexpected redis command ${op}`)};
...
assert.equal(writes.filter(args=>args[0]==='ZADD').length,1,'batch admission must use one ZADD');
assert.equal(commands.length,3,'20-item batch must execute 1 MGET + 1 MSET + 1 ZADD');
console.log(JSON.stringify({flow:'social_queue_batch_admission',items:20,redisReads:reads.length,redisWrites:writes.length,redisCommands:commands.length,redisHttpRequests:commands.length,legacyRedisReads:40,readReductionPercent:97.5}));
