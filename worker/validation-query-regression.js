...
const adminSource=readFileSync(new URL('../app/admin-berita/AdminWorkspace.jsx',import.meta.url),'utf8');assert.match(adminSource,/Cek Google/);assert.match(adminSource,/Cari Artikel/);assert.match(adminSource,/Cek URL/);assert.match(adminSource,/encodeURIComponent\(url\)/);assert.match(adminSource,/encodeURIComponent\(m\.primaryQuery\)/);assert.match(adminSource,/article\.canonicalUrl/);assert.match(adminSource,/_blank/);assert.match(adminSource,/noopener,noreferrer/);
