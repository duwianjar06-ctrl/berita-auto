...
assert.equal(renderStateForRun({status:'failed'},{cardUrls:['x']}),'RENDER_SUCCESS');
assert.equal(renderStateForRun({status:'failed'},null),'RENDER_FAILED');
assert.equal(renderStateForRun({status:'legacy'}),'UNKNOWN_LEGACY');
assert.equal(stageForRun({reason:'cooldown'}),'COOLDOWN');
assert.equal(stageForRun({reason:'already_published'}),'SKIPPED_DUPLICATE');
assert.equal(stageForRun({reason:'queue_empty'}),'NO_CANDIDATE');
assert.equal(stageForRun({status:'retry'},{cardUrls:['x']}),'RETRY_SCHEDULED');
assert.equal(stageForRun({status:'published'}),'SUCCESS');
assert.equal(stageForRun({status:'failed'}),'FAILED');
assert.deepEqual(normalizeSchedulerStatus({vercelCron:true,qstashUsed:false,githubUsed:false}),{vercelCron:'ACTIVE',qstash:'NOT_USED',githubActions:'NOT_USED'});
assert.equal(normalizeSchedulerStatus({vercelCron:false}).qstash,'NOT_USED');
assert.equal(mediaIdMessage({reason:'cooldown',status:'skipped'}),'Belum tersedia — belum melakukan publish');
assert.equal(mediaIdMessage({stage:'PUBLISHING',status:'failed'}),'Belum tersedia — publish gagal');
assert.equal(mediaIdMessage({status:'failed'}),'Belum tersedia — proses belum sampai tahap publish');
assert.match(mediaIdMessage({status:'published'}),/Media ID tidak tercatat/);
...
assert.match(admin,/qstash:'NOT_USED'/);assert.match(admin,/githubActions:scheduler\.githubActions/);assert.match(admin,/INSTAGRAM_MIN_INTERVAL_MINUTES/);assert.match(admin,/readSocialRuns\(100\)/);assert.match(admin,/secretsExposed:false/);
...
assert.match(socialRun,/getSocialFontDiagnostics/);assert.match(socialRun,/cardUrls/);assert.match(socialRun,/failureStage/);assert.match(socialRun,/retryable/);assert.match(socialRun,/nextRetryAt/);assert.match(socialRun,/httpStatus/);assert.match(socialRun,/metaCode/);
assert.match(socialRun,/1080/);assert.match(socialRun,/1350/);assert.match(socialRun,/format:'jpeg'/);
