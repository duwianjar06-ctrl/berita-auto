...
const socialRuntime=await readFile(new URL('../lib/instagram-preparation-runtime.js',import.meta.url),'utf8');
...
const prepareLockMatch=socialRuntime.match(/const LOCK='([^']+)'/);const publishLockMatch=autoUpload.match(/AUTO_LOCK='([^']+)'/);
...
assert.doesNotMatch(socialRuntime,new RegExp(publishLockMatch[1].replace(/[.*+?^${}()|[\]\\]/g,'\\$&')),'prepare must not acquire publisher lock');
...
assert.equal(prepareResult.status,'preparing');assert.equal(publishResult?.queueId,'instagram:ready-1','an active prepare must not hide an existing READY item');
