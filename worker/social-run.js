...
async function validatePublicImage(url) {
  ...
  try {
    ...
    if (!response.ok || !type.startsWith('image/')) {
      throw new Error(`social_card_invalid_response_${response.status}_${type || 'missing_content_type'}`);
    }
    return true;
  } catch (error) {
...
function retryDelay(attempts) {
  return Math.min(30 * 60 * 1000, Math.max(60 * 1000, 2 ** Math.min(attempts, 5) * 60 * 1000));
}
...
async function createAndPublishMedia(article, caption, siteUrl) {
  ...
  if (slides.length === 1) {
    ...
    if (!ready.ready) return { ready, containerId };
    return { ready, containerId, mediaId: await publishMediaContainer(containerId), carousel: false };
  }

  const childIds = [];
  for (const url of urls) {
    const child = await createMediaContainer({ imageUrl: url, isCarouselItem: true });
    childIds.push(child);
  }
  ...
  if (failedChild) return { ready: failedChild, childIds, carousel: true };

  const containerId = await createCarouselContainer({ children: childIds, caption });
  const ready = await pollContainerReady(containerId);
  if (!ready.ready) return { ready, containerId, childIds, carousel: true };
  return { ready, containerId, childIds, mediaId: await publishMediaContainer(containerId), carousel: true };
}
...
export async function runSocialCycle({ trigger = 'manual', now = Date.now() } = {}) {
  ...
  try {
    ...
    // Fast gates come before any queue/article reconciliation or Meta calls.
    ...
    if (shouldSkipDailyLimit(daily, cfg.maxPostsPerDay)) {
      ...
      return { status: 'skipped', reason: 'daily_limit', publishedToday: daily };
    }

    const articles = await readArticles();
    for (const article of articles.slice(0, 100)) {
      if (article?.id && article?.sitePublishedAt) {
        await queueSocialArticle(article).catch(error => console.warn(`[social] queue_reconcile_failed articleId=${article.id} reason=${String(error?.message || error).slice(0, 120)}`));
      }
    }
    ...
    const eligible = queue
      .filter(item => {
        ...
        return article && article.sitePublishedAt && item.state !== 'published';
      })
      .map(item => ({ ...item, article: articles.find(a => a.id === item.articleId) }));

    if (!eligible.length) {
      console.log('[social] skipped reason=queue_empty');
    ...
    if (shouldSkipMetaBuffer(usage, cfg.limitBuffer)) {
      ...
      return { status: 'skipped', reason: 'meta_limit_buffer', remaining: usage.remaining };
    }
    ...
    if (!selected) {
      ...
      return { status: 'skipped', reason: 'no_worthy_content' };
    }
    ...
    try {
      ...
      if (!result.ready.ready) {
        ...
        if (result.ready.permanent) {
          ...
          return { status: 'skipped', reason: 'media_processing_failed' };
        }
        ...
        return { status: 'retry', reason: 'media_processing_pending' };
      }
      ...
      for (let attempt = 1; attempt <= 3; attempt++) {
        try {
          record = await markSocialPublished(processing, { mediaId, publishedAt });
          break;
        } catch (error) {
      ...
      if (!record) {
        ...
        return { status: 'persist_error', articleId: article.id, mediaId };
      }

      let publishedToday = daily + 1;
      try {
        publishedToday = await incrementDailyPublishedCount(now);
      } catch (error) {
        console.warn(`[social] daily_counter_update_failed articleId=${article.id} reason=${String(error?.message || error).slice(0, 120)}`);
      }
      console.log(`[instagram] published articleId=${article.id} mediaId=${mediaId}${result.carousel ? ' type=carousel' : ''}`);
      return {
        status: 'published',
        ...
        mediaId,
        publishedAt,
        carousel: Boolean(result.carousel),
        slideCount: buildSocialSlides(article).length,
        queueRemaining: Math.max(0, eligible.length - 1),
        publishedToday,
        record
      };
    } catch (error) {
      ...
      await markSocialFailure(processing, error, { permanent, retryAfterMs: retryDelay(processing.attempts) });
      const safeStatus = Number(error?.status || 0);
      const safeMetaCode = Number(classification.metaCode || error?.metaCode || 0);
      if (classification.reason === 'auth_token_invalid') {
        console.warn(`[instagram] failed reason=instagram_auth_failed status=${safeStatus} metaCode=${safeMetaCode}`);
      } else {
        console.warn(`[instagram] failed articleId=${article.id} reason=${classification.reason} status=${safeStatus} metaCode=${safeMetaCode}`);
      }
      return { status: permanent ? 'failed' : 'retry', articleId: article.id, reason: classification.reason };
    }
  } finally {
    await releaseLock(SOCIAL_LOCK_KEY, lockToken).catch(() => {});
  }
}

if (process.argv[1]?.endsWith('/worker/social-run.js')) {
  runSocialCycle({ trigger: 'cli' })
    .then(result => console.log(JSON.stringify(result)))
    .catch(error => {
      console.error(`[social] fatal ${String(error?.message || error).slice(0, 240)}`);
      process.exitCode = 1;
    });
}
