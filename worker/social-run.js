...
import {SOCIAL_LOCK_KEY,SOCIAL_LOCK_TTL,socialConfig,queueSocialArticle,readSocialQueue,readRecentPublished,selectBestSocialArticle,deterministicCaption,getDailyPublishedCount,getLastPublishedAt,markSocialProcessing,markSocialFailure,markSocialPublished,incrementDailyPublishedCount} from '../lib/social.js';
...
async function validatePublicImage(url){
  ...
  try{
    ...
    return true;
  }catch(error){throw Object.assign(new Error(String(error?.message||error).slice(0,200)),{kind:'permanent'});}finally{clearTimeout(timer);}
}
...
export async function runSocialCycle({trigger='manual',now=Date.now()}={}){
  ...
  try{
    ...
    const eligible=queue.filter(item=>{const article=articles.find(a=>a.id===item.articleId);return article&&article.sitePublishedAt&&item.state!=='published';}).map(item=>({...item,article:articles.find(a=>a.id===item.articleId)}));
    ...
    if(last&&now-Date.parse(last)<cfg.minIntervalMinutes*60000){console.log('[social] skipped reason=cooldown');return{status:'skipped',reason:'cooldown'}}
    const daily=await getDailyPublishedCount(now);
    if(daily>=cfg.maxPostsPerDay){console.log('[social] skipped reason=daily_limit');return{status:'skipped',reason:'daily_limit',publishedToday:daily}}
    const usage=await getPublishingUsage();
    if(usage.available&&usage.remaining<=cfg.limitBuffer){console.log('[social] skipped reason=meta_limit_buffer');return{status:'skipped',reason:'meta_limit_buffer',remaining:usage.remaining}}
    const selected=selectBestSocialArticle(eligible,{now,recentPublished:recent});
    if(!selected){console.log('[social] skipped reason=no_worthy_content');return{status:'skipped',reason:'no_worthy_content'}}
    const publishedState=await getJson(`ba:social:instagram:published:${selected.articleId}`);
    if(publishedState){console.log(`[social] skipped reason=already_published articleId=${selected.articleId}`);return{status:'skipped',reason:'already_published'}}
    ...
    try{
      ...
      const imageUrl=`${instagramConfig().siteUrl}/api/social-card/${encodeURIComponent(article.id)}`;
      await validatePublicImage(imageUrl);
    ...
    }catch(error){const classification=classifyInstagramError(error);const permanent=classification.kind==='permanent';const retryAfterMs=retryDelay(processing.attempts);await markSocialFailure(processing,error,{permanent,retryAfterMs});if(classification.reason==='auth_token_invalid')console.warn('[instagram] failed reason=instagram_auth_failed');else console.warn(`[instagram] failed articleId=${article.id} reason=${classification.reason}`);return{status:permanent?'failed':'retry',articleId:article.id,reason:classification.reason};}
