...
// Editorial image titles are always wrapped at word boundaries; never slice arbitrary characters.
export function wrapTextByWords(text,{maxWidth=1040,maxLines=3,fontSize=46,fontFamilyFactor=.55,ellipsis=true}={}){
 ...
 const kept=lines.slice(0,maxLines);let remaining=words.join(' ');
 for(let i=0;i<maxLines-1;i++){const line=kept[i];remaining=remaining.slice(line.length).trim()}
