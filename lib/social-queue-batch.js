import {mgetJson,msetJson,sortedAdd} from './persistence.js';
...
export async function queueSocialArticles(articles,{now=new Date().toISOString()}={}){
  ...
  for(const entry of indexEntries)await sortedAdd('ba:social:instagram:queue',entry.score,entry.id);
  return result;
}

export const socialQueueBatchDiagnostics={version:1,redisReads:1,description:'Concurrent social queue admission replaces 2 reads per article with one batched MGET.'};
