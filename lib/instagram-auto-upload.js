...
export async function runInstagramAutoUpload({trigger='scheduled',now=Date.now()}={}){
  ...
  try{
    ...
    if(!immediate&&Number.isFinite(lastAt)&&now-lastAt<interval){const nextAt=new Date(lastAt+interval).toISOString();await bulkEnqueueInstagramPublishItems(queue,{reason:'AUTO_UPLOAD_INTERVAL',status:'QUEUED',now,estimatedResumeAt:nextAt});return{status:'queued',reason:'interval_backoff',nextAt,...base,remainingReady:base.readyFound,durationMs:Date.now()-start};}
    const throttle=await getInstagramPublishThrottle({now});
    if(throttle.active&&!throttle.probeDue){await bulkEnqueueInstagramPublishItems(queue,{reason:'META_ACTION_THROTTLE',status:'WAITING_META',now,estimatedResumeAt:throttle.estimatedResumeAt});base.rateLimited=true;base.actionThrottled=true;base.skipReason='META_ACTION_THROTTLE';base.remainingQueued=queue.length;return{status:'queued',reason:'META_ACTION_THROTTLE',...base,estimatedResumeAt:throttle.estimatedResumeAt,nextProbeAt:throttle.nextProbeAt,durationMs:Date.now()-start};}
    ...
    base.remainingQueued=(await listInstagramPublishQueue()).length;base.remainingReady=(await listInstagramReviewQueue()).filter(isReady).length;
    ...
    if(isMetaActionThrottle(error)){const throttle=await activateInstagramActionThrottle(error,{now:Date.now()}),queue=await listInstagramPublishQueue();await bulkEnqueueInstagramPublishItems(queue,{reason:'META_ACTION_THROTTLE',status:'WAITING_META',now:Date.now(),estimatedResumeAt:throttle.estimatedResumeAt});base.rateLimited=true;base.actionThrottled=true;base.skipReason='META_ACTION_THROTTLE';base.metaErrorCode=9;base.metaErrorSubcode=2207069;base.metaCalls+=Number(error?.metaCalls||1);base.remainingQueued=queue.length;return{status:'partial',reason:'META_ACTION_THROTTLE',...base,estimatedResumeAt:throttle.estimatedResumeAt,nextProbeAt:throttle.nextProbeAt,durationMs:Date.now()-start};}
    base.publishFailed=base.publishAttempted?1:0;base.skipReason=String(error?.message||error).slice(0,240);base.metaCalls+=Number(error?.metaCalls||0);try{base.remainingQueued=(await listInstagramPublishQueue()).length}catch{}return{status:'failed',reason:'publish_failed',...base,durationMs:Date.now()-start};
