...
import {getInstagramPublishGate,getInstagramPublishThrottle,enqueueInstagramPublishItem,listInstagramPublishQueue,prepareQueuedItemForPublish,requeueInstagramPublishItem,publishInstagramReviewItemQueued,clearInstagramPublishThrottle,isMetaActionThrottle,activateInstagramActionThrottle,getInstagramPublishQueueSnapshot} from './instagram-publish-queue.js';
...
export async function runInstagramAutoUpload({trigger='scheduled',now=Date.now()}={}){
  ...
  try{
    ...
    base.readyFound=rows.filter(isReady).length;
    ...
    for(const row of rows.filter(isReady))await enqueueInstagramPublishItem(row,{reason:'AUTO_UPLOAD',status:'QUEUED',now});
    let queue=await listInstagramPublishQueue();
    base.eligibleReady=queue.length;
    base.remainingQueued=queue.length;
    if(!immediate&&Number.isFinite(lastAt)&&now-lastAt<interval){const nextAt=new Date(lastAt+interval).toISOString();for(const row of queue)await enqueueInstagramPublishItem(row,{reason:'AUTO_UPLOAD_INTERVAL',status:'QUEUED',now,estimatedResumeAt:nextAt});return{status:'queued',reason:'interval_backoff',nextAt,...base,remainingReady:base.readyFound,durationMs:Date.now()-start};}
    const throttle=await getInstagramPublishThrottle({now});
    if(throttle.active&&!throttle.probeDue){for(const row of queue)await enqueueInstagramPublishItem(row,{reason:'META_ACTION_THROTTLE',status:'WAITING_META',now,estimatedResumeAt:throttle.estimatedResumeAt});base.rateLimited=true;base.actionThrottled=true;base.skipReason='META_ACTION_THROTTLE';base.remainingQueued=queue.length;return{status:'queued',reason:'META_ACTION_THROTTLE',...base,estimatedResumeAt:throttle.estimatedResumeAt,nextProbeAt:throttle.nextProbeAt,durationMs:Date.now()-start};}
    ...
    queue=await listInstagramPublishQueue();
    if(!gate.allowed){for(const row of queue)await enqueueInstagramPublishItem(row,{reason:gate.reason,status:'WAITING_META',now,estimatedResumeAt:gate.estimatedResumeAt||gate.nextProbeAt||null});base.remainingQueued=queue.length;return{status:'queued',reason:gate.reason,...base,estimatedResumeAt:gate.estimatedResumeAt||gate.nextProbeAt||null,nextProbeAt:gate.nextProbeAt||null,publishingUsage:gate.publishingUsage||null,durationMs:Date.now()-start};}
    ...
    const published=await publishInstagramReviewItemQueued(candidate.queueId,publishInstagramReviewItem,{now,prechecked:true});
    ...
    if(published.status==='queued'){
      ...
      base.remainingQueued=(await listInstagramPublishQueue()).length;
    ...
    if(published.status==='published'){
      ...
      await setJson(LAST_PUBLISH_KEY,{queueId:published.item.queueId,publishedAt:published.item.publishedAt||new Date().toISOString()});
    }
    base.remainingQueued=(await listInstagramPublishQueue()).length;
    base.remainingReady=(await listInstagramReviewQueue()).filter(isReady).length;
    return{status:base.publishPosted?'partial':'skipped',reason:base.publishPosted&&base.remainingQueued?'safe_pacing_remaining':base.publishPosted?'published':'publish_failed_or_stale',...base,item:published.item||null,durationMs:Date.now()-start};
  }catch(error){
    if(isMetaActionThrottle(error)){
      const throttle=await activateInstagramActionThrottle(error,{now:Date.now()});
      const queue=await listInstagramPublishQueue();for(const row of queue)await requeueInstagramPublishItem(row,{reason:'META_ACTION_THROTTLE',estimatedResumeAt:throttle.estimatedResumeAt,now:Date.now()});
      base.rateLimited=true;base.actionThrottled=true;base.skipReason='META_ACTION_THROTTLE';base.metaErrorCode=9;base.metaErrorSubcode=2207069;base.metaCalls+=Number(error?.metaCalls||1);base.remainingQueued=queue.length;
      return{status:'partial',reason:'META_ACTION_THROTTLE',...base,estimatedResumeAt:throttle.estimatedResumeAt,nextProbeAt:throttle.nextProbeAt,durationMs:Date.now()-start};
    }
    base.publishFailed=base.publishAttempted?1:0;base.skipReason=String(error?.message||error).slice(0,240);base.metaCalls+=Number(error?.metaCalls||0);base.remainingQueued=(await listInstagramPublishQueue()).length;
    return{status:'failed',reason:'publish_failed',...base,durationMs:Date.now()-start};
