...
export async function runInstagramAutoUpload({trigger='scheduled',now=Date.now()}={}){
  ...
  try{
    ...
    if(!immediate&&!Number.isNaN(lastAt)&&now-lastAt<interval){return{status:'queued',reason:'interval_backoff',nextAt:new Date(lastAt+interval).toISOString(),...base,remainingReady:base.readyFound,durationMs:Date.now()-start};}
