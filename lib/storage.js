...
async function ensurePersistent(key,loader,label){
  ...
  const imported=hydrate(await loader());
  await setJson(key,imported);
  console.log(`[storage] migrated ${label} count=${imported.length}`);
  return imported;
}

export async function readArticles(){
  if(persistenceConfigured()){
    return ensurePersistent(ARTICLES_KEY,async()=>process.env.VERCEL?readRemote('articles.json'):readJson(articlesPath), 'articles');
  }
  if(process.env.VERCEL)throw new Error('persistent_database_not_configured_for_production');
...
export async function readPending(){
  if(persistenceConfigured()){
    return ensurePersistent(PENDING_KEY,async()=>process.env.VERCEL?readRemote('pending-articles.json'):readJson(pendingPath), 'pending');
  }
  if(process.env.VERCEL)throw new Error('persistent_database_not_configured_for_production');
...
export async function writePending(items){
  ...
  if(persistenceConfigured()){
    await setJson(PENDING_KEY,normalized);
    return normalized;
  }
