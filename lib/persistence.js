...
export async function acquireLock(key,tokenValue,ttlSeconds=120){
  if(!persistenceConfigured()) return false;
  const result=await command(['SET',key,tokenValue,'NX','EX',ttlSeconds]);
  if(result==='OK'&&key==='ba:social:instagram:prepare:lock'&&typeof globalThis!=='undefined')globalThis.__BA_INSTAGRAM_PREPARE_ACTIVE__=true;
  return result==='OK';
}

export async function releaseLock(key,tokenValue){
  if(!persistenceConfigured()) return false;
  const current=await command(['GET',key]);
  if(current!==tokenValue) return false;
  await command(['DEL',key]);
  if(key==='ba:social:instagram:prepare:lock'&&typeof globalThis!=='undefined')globalThis.__BA_INSTAGRAM_PREPARE_ACTIVE__=false;
  return true;
}

// Compatibility bridge for the Instagram review module. During an actual
// preparation run it also performs a bounded repair pass over persisted failed
// review records, so stale failures created by older preparation versions can
// be revalidated without creating a second queue record.
if(typeof globalThis!=='undefined'&&!globalThis.listInstagramReviewQueue){
  globalThis.listInstagramReviewQueue=async()=>{
    if(globalThis.__BA_INSTAGRAM_PREPARE_ACTIVE__){
      try{const {repairInstagramReviewQueue}=await import('./instagram-review-repair.js');await repairInstagramReviewQueue({limit:5});}catch{}
    }
    return persistenceConfigured()?listJson('ba:social:instagram:review:index'):[];
  };
}
