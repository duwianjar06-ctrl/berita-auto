...
// Compatibility bridge for the Instagram review module. The review queue reader
// was introduced during the review-mode refactor while existing call sites still
// referenced the old local helper. Keep the persisted index as the single source
// of truth and expose the bridge only on the server global used by Node/Next.
if(typeof globalThis!=='undefined'&&!globalThis.listInstagramReviewQueue){
  globalThis.listInstagramReviewQueue=async()=>persistenceConfigured()?listJson('ba:social:instagram:review:index'):[];
}
