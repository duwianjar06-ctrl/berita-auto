...
export function sitemapStatus(article={}){const published=String(article.status||'').toUpperCase()==='PUBLISHED';const indexable=isIndexableArticle(article)&&isArticleRouteResolvable(article);const path=article?.slug?articlePath(article):null;const url=path?`${siteUrl()}${path}`:null;return{included:Boolean(published&&indexable&&path),url,lastSitemapCheckAt:new Date().toISOString(),reason:published&&indexable&&path?null:(!published?'not_published':!indexable?'not_indexable':'missing_route')}}
