...
function classifyFailure(result){
  ...
  const articleCode=String(result?.articleCheck?.failureCode||'').toUpperCase();
  ...
  if(reason==='ARTICLE_INVALID'){
    ...
    if(failures.includes('not_published'))return'ARTICLE_NOT_PUBLISHED';
...
function humanAttentionReason(row,reason){
  const code=String(reason||row?.attentionReason||'UNKNOWN');
  const url=row?.canonicalUrl||row?.article?.canonicalUrl||null;
  const map={
    ...
    CANONICAL_URL_TIMEOUT:'Pengecekan halaman berita melewati batas waktu. Artikel tetap diproses ulang secara bounded.',
  ...
  return{code,message:map[code]||code,canonicalUrl:url};
}
function recoverableFailure(code){return code==='CANONICAL_URL_TIMEOUT'||code==='CANONICAL_URL_UNREACHABLE'||code.startsWith('CARD_PUBLIC_')||code==='CARD_PUBLIC_CHECK_FAILED'||code==='CARD_UPLOAD_FAILED'||code==='STORAGE_FAILED';}
...
export async function prepareInstagramReviewQueue({trigger='qstash',now=Date.now()}={}){
  ...
  const diagnostics={runAt:new Date(now).toISOString(),trigger,itemsRead:0,candidateCount:0,candidates:0,eligibleCount:0,candidateAttempted:0,rejectedDuplicate:0,rejectedPosted:0,rejectedRemoved:0,rejectedStale:0,rejectedInvalidArticle:0,rejectedSourceUrl:0,rejectedImage:0,rejectedCard:0,rejectedCaption:0,storageFailure:0,fallbackUsed:0,preparedCount:0,readyCreated:0,canonicalPass:0,canonicalTimeout:0,canonical404:0,canonicalOther:0,cardRenderPass:0,cardPersistPass:0,cardPublicPass:0,cardPublicTimeout:0,cardPublic404:0,cardPublic5xx:0,cardPublicOther:0,attentionCreated:0,attentionRecovered:0,skippedBackoff:0,repairAttempted:0,repairStillInvalid:0,repairError:null,metaCalls:0};
  try{
    let repairSummary=null;
    try{
      repairSummary=await repairInstagramReviewQueue({limit:5,now});
      diagnostics.attentionRecovered=Number(repairSummary?.recovered||0);
      diagnostics.repairAttempted=Number(repairSummary?.attempted||0);
      diagnostics.repairStillInvalid=Number(repairSummary?.stillInvalid||0);
      diagnostics.skippedBackoff=Number(repairSummary?.skippedBackoff||0);
    }catch(error){diagnostics.repairError=String(error?.message||error).slice(0,240);console.error('[instagram-review] repair pass failed',error);}
    ...
    diagnostics.candidates=eligible.length;
    ...
    const picked=ranked.slice(0,PREPARATION_ATTEMPT_BUDGET);
    const prepared=[];
    for(let i=0;i<picked.length;i++){
      if(diagnostics.readyCreated>=PREPARATION_READY_TARGET)break;
      ...
      try{await upsertIndexed(REVIEW_QUEUE_INDEX,key(queueId),base);}catch(error){diagnostics.storageFailure++;const failed={...base,status:'FAILED',attentionReason:'STORAGE_FAILED',lastError:String(error?.message||error).slice(0,240),updatedAt:new Date().toISOString(),attemptCount:1,nextRetryAt:new Date(now+retryDelayMs(1)).toISOString()};try{await upsertIndexed(REVIEW_QUEUE_INDEX,key(queueId),failed);}catch{};prepared.push(failed);diagnostics.attentionCreated++;await notifyAttention(failed,'STORAGE_FAILED');continue;}
