...
export async function prepareInstagramReviewQueue({trigger='qstash',now=Date.now()}={}){
  ...
  try{
    ...
    const result={status:'prepared',ready:(await listInstagramReviewQueue()).filter(isReady).length,preparing:prepared.filter(x=>x.status==='PREPARING').length,failed:prepared.filter(x=>x.status==='FAILED').length,prepared:prepared.length,newlyReady:newlyReady.length,metaCalls:0,trigger,diagnostics,notification};
