...
export async function prepareInstagramReviewQueue({trigger='qstash',now=Date.now()}={}){
  ...
  const diagnostics={runAt:new Date(now).toISOString(),trigger,itemsRead:0,candidateCount:0,eligibleCount:0,rejectedDuplicate:0,rejectedPosted:0,rejectedRemoved:0,rejectedStale:0,rejectedInvalidArticle:0,rejectedSourceUrl:0,rejectedImage:0,rejectedCard:0,rejectedCaption:0,storageFailure:0,fallbackUsed:0,preparedCount:0,readyCreated:0,attentionCreated:0,attentionRecovered:0,repairAttempted:0,repairStillInvalid:0,repairError:null,metaCalls:0};
  try{
    let repairSummary=null;
    try{
      repairSummary=await repairInstagramReviewQueue({limit:5});
      ...
      diagnostics.repairError=String(error?.message||error).slice(0,240);
      console.error('[instagram-review] repair pass failed',error);
    }
    ...
    for(let i=0;i<picked.length;i++){
      ...
      if(row.status==='READY')diagnostics.readyCreated++;
        ...
        if(failureReason==='ARTICLE_NOT_PUBLISHED'||failureReason.startsWith('ARTICLE_')||failureReason.startsWith('CANONICAL_'))diagnostics.rejectedInvalidArticle++;
        else if(failureReason==='SOURCE_URL_FAILED'||failureReason==='SOURCE_URL_INVALID')diagnostics.rejectedSourceUrl++;
