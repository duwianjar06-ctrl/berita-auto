...
export function normalizeArticleStatus(value){const v=String(value||'').trim().toUpperCase();return ARTICLE_STATUSES.includes(v)?v:'DRAFT'}
export function articleIsIndexable(article={}){return article.contentType==='article'&&article.status==='PUBLISHED'&&article.indexable!==false&&String(article.robots||'index, follow').toLowerCase().startsWith('index')}
