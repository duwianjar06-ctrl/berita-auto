...
export async function getInstagramAutomationSettings(){
  ...
  return {autoUploadEnabled:stored?.autoUploadEnabled===true,autoUploadIntervalMinutes:Math.max(15,Number(stored?.autoUploadIntervalMinutes||DEFAULT_AUTO_UPLOAD_INTERVAL_MINUTES)),updatedAt:stored?.updatedAt||null,updatedBy:stored?.updatedBy||null,source:stored?'persistent':'default'};
}
