...
export async function repairInstagramReviewQueue({limit=MAX_REPAIRS,now=Date.now()}={}){
  ...
  const diagnostics={examined:rows.length,attempted:0,recovered:0,stillInvalid:0,articleNotFound:0,canonical200:0,imageFallback:0,cardFailed:0,captionFailed:0,storageFailed:0,skippedBackoff:0};
  const repaired=[];
