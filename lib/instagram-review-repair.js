...
export async function repairInstagramReviewItem(queueId,{now=Date.now()}={}){
  const row=await getJson(KEY(String(queueId)));
  if(!row)return{status:'not_found',queueId:String(queueId)};
  if(!repairable(row,now))return{status:'skipped',queueId:String(queueId),reason:'not_due',item:row};
  ...
  const originalFailure={
    ...
    failedSlide:row.failedSlide||null
  };

  if(!article){
    const updated=await persist(row,{status:'FAILED',attentionReason:'ARTICLE_NOT_FOUND',failureCode:'ARTICLE_NOT_FOUND',failureStage:'ARTICLE_LOOKUP',failureMessage:human('ARTICLE_NOT_FOUND'),articleFound:false,articlePublished:false,repairAttempted:true,repairResult:'FAILED',failedAt:new Date().toISOString(),nextRetryAt:null});
    return{status:'failed',queueId:row.queueId,recovered:false,item:updated};
  }
  ...
  // Revalidate the persisted live card first for stale public-card failures.
  const priorCardFailure=String(row.failureCode||row.attentionReason||'').toUpperCase();
  const isStaleCardFailure=priorCardFailure==='CARD_PUBLIC_CHECK_FAILED'||priorCardFailure.startsWith('CARD_PUBLIC_');
  const candidateInput={article};
  if(isStaleCardFailure&&Array.isArray(row.cardUrls)&&row.cardUrls.length){
    candidateInput.cardUrls=row.cardUrls;
    candidateInput.previewUrl=row.previewUrl||row.cardUrls[0]||null;
  }

  const result=await prepareInstagramCandidate(candidateInput,{siteUrl:siteUrl(),runId:`repair-${Date.now()}-${row.queueId}`,full:true});
  const failureCode=classify(result);
  const failedSlideIndex=Array.isArray(result.cardChecks)?result.cardChecks.findIndex(item=>item?.failureCode||item?.valid===false):-1;
  const failedSlide=failedSlideIndex>=0?failedSlideIndex+1:null;
  ...
  if(result.status==='READY'){
    ...
    const updated=await persist(row,{
      ...
      status:'READY',preparationState:'READY',readyAt:result.readyAt||new Date().toISOString(),lastError:null,attentionReason:null,failureCode:null,failureStage:null,failureMessage:null,failureDetail:null,failedSlide:null,failedAt:null,repairAttempted:true,repairResult:'SUCCESS',resolvedFailure,
    ...
    return{status:'recovered',queueId:row.queueId,recovered:true,item:updated,oldFailure:originalFailure};
  }
  ...
  const updated=await persist(row,{
    title:article.title,category:article.category,stableId:stableId||row.stableId,canonicalUrl:canonicalUrl||row.canonicalUrl,
    attemptedCanonicalUrl:result.articleCheck?.canonicalUrl||canonicalUrl||row.attemptedCanonicalUrl||null,
    ...
    status:'FAILED',attentionReason:failureCode,failureCode,failureStage:result.cardFailureStage||result.articleCheck?.failureStage||'PREPARATION',failureMessage:human(failureCode),lastError:failureCode,
    failedSlide,repairAttempted:true,repairResult:'FAILED',failedAt:new Date().toISOString(),attemptCount,nextRetryAt
  });
  return{status:'failed',queueId:row.queueId,recovered:false,item:updated,oldFailure:originalFailure};
}

export async function repairInstagramReviewQueue({limit=MAX_REPAIRS,now=Date.now(),queueId=null}={}){
  if(queueId)return repairInstagramReviewItem(queueId,{now});
  ...
  for(const row of rows){
    ...
    try{
      const result=await repairInstagramReviewItem(row.queueId,{now});
      if(result.status==='skipped'){diagnostics.skippedBackoff++;continue;}
      if(result.recovered)diagnostics.recovered++;
      else{
        diagnostics.stillInvalid++;
        const code=String(result.item?.failureCode||'');
        if(code==='ARTICLE_NOT_FOUND')diagnostics.articleNotFound++;
        if(code.startsWith('CARD_'))diagnostics.cardFailed++;
        if(code==='CAPTION_FAILED')diagnostics.captionFailed++;
      }
      if(result.item)repaired.push(result.item);
    }catch(error){
      diagnostics.stillInvalid++;diagnostics.storageFailed++;
      console.error('[instagram-review-repair] item failed',row.queueId,error);
    }
  }
  const summary={...diagnostics,repairedAt:new Date().toISOString(),items:repaired.map(row=>({queueId:row.queueId,status:row.status,failureCode:row.failureCode||null,repairResult:row.repairResult||null,canonicalUrl:row.canonicalUrl||null,failedSlide:row.failedSlide||null,nextRetryAt:row.nextRetryAt||null}))};
