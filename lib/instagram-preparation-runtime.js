...
export async function prepareInstagramProductionQueue({trigger='social-prepare',now=Date.now()}={}){
  ...
  if(!(await acquireLock(LOCK,token,120)))return{status:'skipped',reason:'lock_busy',metaCalls:0};
  const diagnostics={runAt:new Date(now).toISOString(),trigger,queueTotal:0,existingEligible:0,freshEligible:0,candidateAttempted:0,skippedBackoff:0,skippedRemoved:0,skippedPosted:0,skippedPublishing:0,skippedDuplicate:0,canonicalPass:0,canonicalTimeout:0,canonical404:0,canonicalOther:0,imageValid:0,fallbackUsed:0,cardRenderPass:0,cardPersistPass:0,cardPublicPass:0,cardPublicTimeout:0,cardPublic404:0,cardPublic5xx:0,cardPublicOther:0,captionValid:0,captionFailed:0,attentionCreated:0,attentionRecovered:0,readyCreated:0,readyTotal:0,preparedCount:0,repairAttempted:0,repairStillInvalid:0,skippedBackoffRepair:0,metaCalls:0,primaryBlocker:null};
  try{
    try{const repair=await repairInstagramReviewQueue({limit:5,now});diagnostics.attentionRecovered=Number(repair?.recovered||0);diagnostics.repairAttempted=Number(repair?.attempted||0);diagnostics.repairStillInvalid=Number(repair?.stillInvalid||0);diagnostics.skippedBackoffRepair=Number(repair?.skippedBackoff||0);}catch(error){console.error('[instagram-prepare] repair pass failed',error);}
    ...
    for(const item of existingPool){const id=String(item.article.id);const prior=existing.find(row=>String(row.articleId)===id);if(prior?.nextRetryAt&&Date.parse(prior.nextRetryAt)>now){diagnostics.skippedBackoff++;continue;}if(await getJson(`ba:social:instagram:published:${id}`)){diagnostics.skippedPosted++;continue;}if(await getJson(REMOVED(id))){diagnostics.skippedRemoved++;continue;}if(prior&&['READY','PREPARING','PUBLISHING','POSTED','REMOVED'].includes(prior.status)){diagnostics.skippedDuplicate++;continue;}const selected=selectBestSocialArticle([item],{now,recentPublished:recent});if(selected)rank(selected,existingRanked);}
    for(const item of freshPool){const id=String(item.article.id);if(await getJson(`ba:social:instagram:published:${id}`)){diagnostics.skippedPosted++;continue;}if(await getJson(REMOVED(id))){diagnostics.skippedRemoved++;continue;}const selected=selectBestSocialArticle([item],{now,recentPublished:recent});if(selected)rank(selected,freshRanked);}
    ...
    for(let i=0;i<picked.length&&diagnostics.readyCreated<READY_TARGET;i++){
      diagnostics.candidateAttempted++;const item=picked[i];const article=item.article;const queueId=`${article.id}:${article.stableId||article.id}`;const preparedAt=new Date().toISOString();const canonicalUrl=`${siteUrl()}${articlePath(article)}`;
      ...
      try{const result=await prepareInstagramCandidate(item,{siteUrl:siteUrl(),runId:`production-${now}-${i+1}`,full:true});const code=result.status==='READY'?null:failureCode(result);if(result.articleCheck?.valid)diagnostics.canonicalPass++;else if(code==='CANONICAL_URL_TIMEOUT')diagnostics.canonicalTimeout++;else if(code==='CANONICAL_URL_404')diagnostics.canonical404++;else if(String(code).startsWith('CANONICAL_'))diagnostics.canonicalOther++;if(result.image?.status==='FALLBACK_USED')diagnostics.fallbackUsed++;else diagnostics.imageValid++;if(result.render?.status==='RENDER_SUCCESS')diagnostics.cardRenderPass++;if(result.cardUrls?.length)diagnostics.cardPersistPass++;if(result.cardReady&&!result.cardFailureCode)diagnostics.cardPublicPass++;if(result.cardFailureCode==='CARD_PUBLIC_TIMEOUT')diagnostics.cardPublicTimeout++;else if(result.cardFailureCode==='CARD_PUBLIC_404')diagnostics.cardPublic404++;else if(result.cardFailureCode==='CARD_PUBLIC_5XX')diagnostics.cardPublic5xx++;else if(result.cardFailureCode)diagnostics.cardPublicOther++;if(result.captionStatus==='READY')diagnostics.captionValid++;else diagnostics.captionFailed++;const prior=existing.find(row=>String(row.articleId)===String(article.id));const attempt=Number(prior?.attemptCount||0)+1;const nextRetryAt=code&&recoverable(code)?new Date(now+retryMs(attempt)).toISOString():null;const row={...base,status:result.status==='READY'?'READY':'FAILED',preparationState:result.preparationState||result.status,readyAt:result.readyAt||null,cardUrls:result.cardUrls||[],previewUrl:result.previewUrl||result.cardUrls?.[0]||null,cardChecks:result.cardChecks||[],render:result.render||null,imageValidation:result.image||null,imageValidationStatus:result.imageValidationStatus||result.image?.status||'UNKNOWN',imageRelationship:result.imageRelationship||result.image?.status||'UNKNOWN',sourceImageUrl:result.image?.imageUrl||null,imageOriginUrl:result.image?.imageOriginUrl||null,caption:prior?.captionEdited?prior.caption:deterministicCaption(article,siteUrl()),captionOriginal:prior?.captionOriginal||deterministicCaption(article,siteUrl()),captionEdited:Boolean(prior?.captionEdited),captionEditedAt:prior?.captionEditedAt||null,captionStatus:'READY',dedupeStatus:'NOT_POSTED',failureCode:code,failureStage:result.cardFailureStage||result.articleCheck?.failureStage||null,failureMessage:code||null,attentionReason:code,lastError:code,attemptCount:result.status==='READY'?0:attempt,nextRetryAt,failedAt:result.status==='READY'?null:new Date().toISOString(),updatedAt:new Date().toISOString()};await upsertIndexed(INDEX,ITEM(queueId),row);prepared.push(row);diagnostics.preparedCount++;if(row.status==='READY'){diagnostics.readyCreated++;}else{diagnostics.attentionCreated++;await notifyAttention(row,code);}}
      catch(error){const code='STORAGE_FAILED';const row={...base,status:'FAILED',failureCode:code,failureStage:'PREPARATION',failureMessage:String(error?.message||error).slice(0,240),attentionReason:code,lastError:code,attemptCount:1,nextRetryAt:new Date(now+retryMs(1)).toISOString(),updatedAt:new Date().toISOString()};try{await upsertIndexed(INDEX,ITEM(queueId),row);}catch{}prepared.push(row);diagnostics.attentionCreated++;await notifyAttention(row,code);}
    }
    const all=await listJson(INDEX);diagnostics.readyTotal=all.filter(isReady).length;diagnostics.primaryBlocker=primaryBlocker(diagnostics);const newlyReady=prepared.filter(isReady);const notification=await notifyReady(newlyReady);const result={status:'prepared',metaCalls:0,trigger,ready:diagnostics.readyTotal,readyTotal:diagnostics.readyTotal,preparing:prepared.filter(x=>x.status==='PREPARING').length,failed:prepared.filter(x=>x.status==='FAILED').length,prepared:prepared.length,newlyReady:newlyReady.length,diagnostics,notification};await setJson('ba:social:instagram:prepare:last-run',result);return result;
  }finally{await releaseLock(LOCK,token).catch(()=>{});}
}
