...
function validateOutput(output,material=''){
  ...
  const sourceText=cleanText(material||'');
  ...
  if(content.length<Math.max(320,sourceText.length*.35))throw new Error('content_too_short');
...
function fallback(item,material=''){
  ...
  return {
    title:cleanText(item.title||'Berita terbaru'),
    excerpt:cleanExcerpt(item.summary||item.title||''),
    content:body.join('\n\n'),
...
async function generateGemini(item,material){
  ...
  const payload={
    contents:[{parts:[{text:buildPrompt(item,material)}]}],
    generationConfig:{
      temperature:0.15,
...
async function tryProvider(name,fn,item,material){
  try{
    const result=await fn(item,material);
    if(result){
      console.log(`[ai] provider=${name} status=success model=${result.generationModel}`);
      return result;
    }
    return null;
  }catch(error){
    const status=providerError(name,error);
...
export function getAIProviderConfig(){
  return {
    primary:'gemini',
    gemini:{configured:Boolean(process.env.GEMINI_API_KEY),model:process.env.GEMINI_MODEL||DEFAULT_GEMINI_MODEL},
    openai:{configured:Boolean(process.env.OPENAI_API_KEY),model:process.env.OPENAI_MODEL||DEFAULT_OPENAI_MODEL},
...
export async function generateArticle(item,materialArg=''){
  ...
  const gemini=await tryProvider('gemini',generateGemini,item,material);
  if(gemini)return gemini;
  const openai=await tryProvider('openai',generateOpenAI,item,material);
  if(openai)return openai;
  const result=fallback(item,material);
  console.warn('[ai] provider=fallback status=used');
  return result;
}
